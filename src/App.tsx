...
export default function App() {
  ...
  // Fetch all contacts in the background once on load
  useEffect(() => {
    const fetchAllProfiles = async () => {
      const allChats = conversationsRef.current || [];
      const MAX_CONCURRENT = 3;
      for (let i = 0; i < allChats.length; i += MAX_CONCURRENT) {
        const batch = allChats.slice(i, i + MAX_CONCURRENT);
        await Promise.all(batch.map(async chat => {
          if (!fetchedProfilesRef.current.has(chat.id)) {
  ...
  const applyQuickReply = (qr: QuickReply) => {
    const orderHistory = orderHistories[activeChat?.id || ''];
    let sapaan = '';
    if (orderHistory?.orders?.[0]?.nama) {
      sapaan = orderHistory.orders[0].nama;
    } else if (orderHistory?.name && orderHistory.name !== 'Me') {
      sapaan = orderHistory.name;
    } else if (activeChat?.name && activeChat.name !== activeChat.phone && !activeChat.name.startsWith('+')) {
      sapaan = activeChat.name;
    }
                          ...
                          .map(qr => (
                            ...
                            >
                              ...
                              <p className="text-sm text-slate-600 line-clamp-2 leading-snug">
                                {qr.message.replace(/\{\{nama\}\}/gi, orderHistories[activeChat?.id || '']?.orders?.[0]?.nama || orderHistories[activeChat?.id || '']?.name || (activeChat?.name && !activeChat.name.startsWith('+') ? activeChat.name : '{{nama}}'))}
                              </p>
                            </button>
                          ))
                        }
